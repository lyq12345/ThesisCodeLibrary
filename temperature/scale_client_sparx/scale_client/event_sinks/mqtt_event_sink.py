...
class MQTTEventSink(MqttApplication, EventSink):
    ...
    def send_event(self, event):
        ...
        # static topic format
        #self._topic = "iot-1/yuqiaol5/temperature/%s/json"
        topic = self._topic % topic_event_type

        # static topic name
        #topic = "iot-1/yuqiaol5/temperature"
	...
	#log.debug("Checking availibility of event %s" % event)
        if not self.is_connected:
	    log.info("Cannot connect to MQTT broker")
            if not self._try_connect():
                log.error("MQTT publisher failure: Cannot connect")
                return False
	#log.debug("Super %s, self %s" % (super(MQTTEventSink, self),self))
	#log.debug("Is connected: %s" % self.is_connected)
	#log.debug("Avalibility: %s" % super(MQTTEventSink, self).check_available(event))
