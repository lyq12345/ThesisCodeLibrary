...
def calculate_standard_deviation(data):
    ...
    if std_deviation < threshold:
        ...
        print("变量波动较大")
data = pd.read_csv('../power/evaluation.csv')
# y = data['qos']
# x1 = data['cpu']
# x2 = data['memory']
x3 = data['power']
...
popt, pcov = curve_fit(power_model, x4_freq, x3, maxfev=10000)

a_fit, b_fit, c_fit = popt
print(a_fit, b_fit, c_fit)

# 评估拟合效果
residuals = x3 - power_model(x4_freq, a_fit, b_fit, c_fit)
ss_res = np.sum(residuals**2)
ss_tot = np.sum((x3 - np.mean(x3))**2)
...
# 绘制原始数据和拟合曲线
plt.scatter(x4_freq, x3, label='Original Data')
